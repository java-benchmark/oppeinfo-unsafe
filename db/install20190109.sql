...
declare r record;
				...
				i integer:=0;
BEGIN
	for r in (select count(*), lpm.lesson_plan_id, lpm.curriculum_version_omodule_id
							from lesson_plan_module lpm
							group by lpm.lesson_plan_id, lpm.curriculum_version_omodule_id
							having count(*) > 1)
	LOOP
		i:=0;
		for rr in (select *
							 from lesson_plan_module
							 where lesson_plan_id=r.lesson_plan_id and curriculum_version_omodule_id=r.curriculum_version_omodule_id)
		LOOP
			if i=0 THEN
				i:=rr.id;
			else
				update journal_omodule_theme set lesson_plan_module_id=i where lesson_plan_module_id=rr.id;
				delete from lesson_plan_module where id=rr.id;
			end if;
		end loop;
	end loop;
end;
...
create unique index UQ_lesson_plan_module on lesson_plan_module (lesson_plan_id, curriculum_version_omodule_id);

with s as (select lpp.id, sg.curriculum_version_id
from student_group sg
		 join lesson_plan lpp on sg.id=lpp.student_group_id
		 join study_year sy on lpp.study_year_id=sy.id and sy.start_date > to_date('01.08.2018','dd.mm.yyyy')
where sg.curriculum_version_id!=lpp.curriculum_version_id)
update lesson_plan lp set curriculum_version_id=s.curriculum_version_id 
from s
where lp.id=s.id;

delete from lesson_plan_module lpp
where (select count(*) from journal_omodule_theme jot where lpp.id=jot.lesson_plan_module_id)=0 and lpp.teacher_id is null;

update classifier set is_vocational=true where code='AKADPUHKUS_POHJUS_O';
...
CREATE OR REPLACE FUNCTION upd_curriculum_completion_trgr()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
declare i integer;
...
create trigger student_curriculum_completion_trg after update on student for each row execute procedure upd_curriculum_completion_trgr();

CREATE OR REPLACE FUNCTION upd_timetable_trgr()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
declare i integer;
BEGIN
	with s as (select te.* 
						 from journal jj
								 join timetable_object tob on jj.id=tob.journal_id
								 join timetable_event te on tob.id=te.timetable_object_id 
						 where jj.id=NEW.id)
		update timetable_event te set name=coalesce(NEW.name_et,te.name) 
		from s
		where te.id=s.id;
  return null;
end;
...
create trigger journal_upd_trg after update on journal for each row execute procedure upd_timetable_trgr();
